...
def resolve_text_and_label_cols(df: pd.DataFrame, cfg_model: Dict[str, Any]) -> (str, str):
    # texto
    text_col = cfg_model.get("text_col", "review_text_clean")
...
class WeightedTrainer(Trainer):
    ...
    def _maybe_build_log_prior(self, logits: torch.Tensor):
        ...
        self._log_prior = torch.tensor(np.log(priors), dtype=logits.dtype, device=logits.device)
...
def unwrap_for_save(m):
    # torch.compile suele dejar _orig_mod
    if hasattr(m, "_orig_mod"):
        return m._orig_mod
    # accelerate / DDP
    try:
        from transformers.trainer_pt_utils import unwrap_model
...
def main():
    ...
    # TOKENIZERS_PARALLELISM según num_proc
    if int(m.get("tokenize_num_proc", 1)) > 1:
    ...
    if run:
        ...
        try:
            ...
            mlflow.start_run(run_name=f"{base_name}_{timestamp}", tags=ml.get("tags"))
            print(f"[mlflow] Started")
        except Exception:
            run = False

    # --------- Limpieza de salida si se pide --------- #
    out_dir = args.model_out
    clean_output = bool(m.get("clean_output", False))
    if clean_output and os.path.isdir(out_dir):
    ...
    cw_mode = str(cw_cfg.get("mode", "auto")).lower()
    ...
    # torch.compile (opcional en TRAIN)
    ...
    # --------- ENTRENAR (con reanudación robusta) --------- #
    resume_flag = bool(m.get("resume_from_last", False))

    # Si limpiamos la carpeta, no podemos reanudar
    if clean_output and resume_flag:
        print("[warn] clean_output=true ⇒ desactivo resume_from_last")
        resume_flag = False
    ...
    if resume_flag:
        ...
        if last_ckpt is None:
            print(f"[warn] No hay checkpoints en {out_dir}. Empezando desde cero.")
            resume_flag = False

    train_result = trainer.train(resume_from_checkpoint=last_ckpt if resume_flag else None)
    ...
    # --------- FREEZE / EXPORT limpio para infer --------- #
    model_to_save = unwrap_for_save(trainer.model)
    frozen_dir = os.path.join(out_dir, "frozen")
    os.makedirs(frozen_dir, exist_ok=True)

    # Metadatos de etiquetas legibles
    model_to_save.config.id2label = ID2LABEL
    model_to_save.config.label2id = LABEL2ID

    # Guardado seguro
    try:
        model_to_save.save_pretrained(frozen_dir, safe_serialization=True)
    except Exception:
        model_to_save.save_pretrained(frozen_dir)
    tok.save_pretrained(frozen_dir)

    print(f"[freeze] Modelo exportado limpio en: {frozen_dir}")
